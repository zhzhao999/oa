...
menu.active('#salary-level-list');
	$(function(){
	    ...
	    function getList() {
	        $.ajax({
	            url: '${ctx }/salary/dict/level/showListByPage',
	            type: 'POST',
	            dataType: 'json',
	            data: {
	            	type_name: type_name,
	    ...
	    function createTab(list) {
	        ...
	        for (var i = 0, len = data.length; i < len; i++) {
	            html += '<tr>' +
	            	...
	            	'<td>' + data[i].basic_pay + '</td>' +
	            	'<td>' + data[i].oil_subsidy + '</td>' +
	            	'<td>' +
	            		'<div class="btn-group" role="group" aria-label="...">' +
	            			'<a id="look" href="${ctx }/salary/dict/level/showUpdate/' + data[i].id + '" class="btn btn-default btn-sm">修改</a>' +
	            		'</div>' +
            		'</td>'+
	            	'</tr>';
	        }
